...
def download_excel():
    ...
    excel_file = os.path.join("data", file_name)
...
def format_print(r):
    ...
    return [int(r[0]), r[1], str(r[2]).replace(".0", ""), r[3], r[4], r[5], \
        ...
        "" if r[7] == "" else parse_date(r[7]).strftime("%Y-%m-%d"), \
        "" if r[8] == "" else parse_date(r[8]).strftime("%Y-%m-%d"), \
        "" if r[9] == "" else parse_date(r[9]).strftime("%Y-%m-%d"), \
...
if __name__ == "__main__":
    ...
    print("strat to exctract data...")
    data_list = extract_data(parse_excel(excel_file))
    print("************************************************************************************************")
    csv_file = os.path.join("data", excel_date + ".csv")
