...
def get_ma20_of_one(k_data_one):
    data_to_be_inserted = []
    k_list = [ k["close"] for k in k_data_one ]
    for k in k_data:
...
def get_ma20_of_slice(code_list):
    ...
    pool = Pool()
    result = pool.map(get_ma20_of_slice, k_data_group)
    pool.close()
    pool.join()
...
def bulk_get_ma20_and_insert():
    ...
    for i in range(len(slice)):
        result = get_ma20_of_slice(slice[i])
